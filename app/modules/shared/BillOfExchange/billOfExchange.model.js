const billOfExchangeFile = require("../../projects/models/billOfExchange.model");

function addBillOfExchangeFile(project, callback) {
    
    billOfExchangeFile.create(project.billOfExchange, (err, res) => {
        if (err) {
            
            callback(err, null);
        } else if (res) {
            
            callback(null, res);
        } else {
            callback(null, null);
        }
    });
}


async function getBillOfExchange(user, callback) {
    
    var temp=['0','-1'];
    var DATA=[];
    for (let index = 0; index < temp.length; index++) {
        const element = temp[index];
        await billOfExchangeFile.find({ userId: user.userId,file:user.file,deleteflag:element}).populate('pipo').sort({'_id': -1}).then((getuser)=> {
            
            if (getuser.length!= 0) {
                for (let j = 0; j < getuser.length; j++) {
                    DATA.push(getuser[j]);   
                }
            }
            if((index+1) == temp.length){
                callback(null,DATA);
            }
        });
    }
  
}


function getSingleBillOfExchange(user, callback) {
    
    billOfExchangeFile.find({ billExchangeNumber: user.userId }, function(err, user) {
        
        
        if (err) {
            
            callback(err, null);
        } else if (user) {
            
            callback(null, user);
        } else {
            callback(null, null);
        }
    });
}

function updateBillOfExchange(id, bene, callback) {
    billOfExchangeFile.updateOne({
            _id: id,
        }, { $set: bene },
        function(err, user) {
            
            if (err) {
                
                callback(err, null);
            } else if (user) {
                
                callback(null, user);
            } else {
                callback(null, null);
            }
        }
    );
}

module.exports = {
    addBillOfExchangeFile: addBillOfExchangeFile,
    getBillOfExchange: getBillOfExchange,
    getSingleBillOfExchange: getSingleBillOfExchange,
    updateBillOfExchange: updateBillOfExchange,
};
