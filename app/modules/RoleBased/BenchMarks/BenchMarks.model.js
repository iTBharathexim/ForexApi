const BenchMarksModel = require('./BenchMarksSchema.js').BenchMarksModel;
...
const addUser = (user) => {
    return new Promise((resolve, reject) => {
        BenchMarksModel.create(user, (err, res) => {
            if (err) {
                reject(err);
            } else if (res) {
...
const getUser = (userId) => {
    return new Promise((resolve, reject) => {
        BenchMarksModel.find({...userId}, (err, res) => {
            if (err) {
                resolve(err);
            }
            resolve(res);
...
const updateUserById = async (id, data) => {
    return new Promise((resolve, reject) => {
        delete data?.REPO_RATE?._id
        try {
            BenchMarksModel.updateOne({ _id: mongoose.Types.ObjectId(id) }, { $set: data }, (err, res) => {
                if (err) {
                    resolve({ result: res, msg: 'Somethings wrong...', status: false })
                }
                

                BenchMarksModel.find({ _id: mongoose.Types.ObjectId(id) }, (err, res1) => {
...
const updateUserByEmail = async (id, data) => {
    let result = BenchMarksModel.updateOne({ emailId: id }, { $set: data });
    return result
}
...
function OTP_SEND_EMAIL(req) {
    return new Promise((resolve, reject) => {
        if (req.body.emailId) {
            BenchMarksModel.updateOne({
                emailId: req.body.emailId
            }, { $set: { "emailIdVerified": false } }, function (err, user) {
                BenchMarksModel.findOne({
                    emailId: req.body.emailId
