const FXMarginDetailsModel = require('./FXMarginDetailsSchema').FXMarginDetailsModel;
...
const addUser = (user) => {
    return new Promise((resolve, reject) => {
        FXMarginDetailsModel.create(user?.data, (err, res) => {
            if (err) {
                reject(err);
            } else if (res) {
                FXMarginDetailsModel.find({ userId: user?.data?.userId }, (err, res1) => {
                    if (err) {
                        resolve(err);
                    }
                    resolve({ status: true, result: res1, msg: "Created SucessFully...." });
                });
            } else {
                reject(null);
            }
...
const getUser = (userId) => {
    return new Promise((resolve, reject) => {
        FXMarginDetailsModel.find({ userId: userId?.userId }, (err, res) => {
            if (err) {
                resolve(err);
            }
            resolve(res);
...
const getUserDetails = () => {
    return new Promise((resolve, reject) => {
        FXMarginDetailsModel.find({}, (err, res) => {
            if (err) {
                resolve(err);
            }
            resolve(res);
...
const updateUserById = async (id, data) => {
    return new Promise((resolve, reject) => {
        FXMarginDetailsModel.updateOne({ userId: id }, { $set: data }, (err, res) => {
            if (err) {
                resolve({ result: res, msg: 'Somethings wrong...', status: false })
            }
            FXMarginDetailsModel.find({ userId: id }, (err, output) => {
                if (err) {
                    resolve({ result: res, msg: 'Somethings wrong...', status: false })
                }
                resolve({ result: output, msg: 'Sucessfully Update', status: true })
            });
        });
    });
}
...
const updateUserByEmail = async (id, data) => {
    let result = FXMarginDetailsModel.updateOne({ emailId: id }, { $set: data });
    return result
}
...
function OTP_SEND_EMAIL(req) {
    return new Promise((resolve, reject) => {
        if (req.body.emailId) {
            FXMarginDetailsModel.updateOne({
                emailId: req.body.emailId
            }, { $set: { "emailIdVerified": false } }, function (err, user) {
                FXMarginDetailsModel.findOne({
                    emailId: req.body.emailId
