const CouponCodeProductModel = require('./CouponCodeProductSchema.js').CouponCodeProductModel;
...
const addUser = (user) => {
    ...
    return new Promise((resolve, reject) => {
        CouponCodeProductModel.find({ emailId: emailId }, (err, res) => {
            ...
            if (res.length == 0) {
                ...
                CouponCodeProductModel.create(user, (err, res) => {
                    if (err) {
                        reject(err);
                    } else if (res) {
    ...
    return new Promise((resolve, reject) => {
        validators.decodeAuthString(authString, (email, password) => {
            if (email && password) {
                ...
                CouponCodeProductModel.find({ emailId: email }, (err, res) => {
...
const TradeAppLoginMPIN = (email, password) => {
    return new Promise((resolve, reject) => {
        CouponCodeProductModel.find({ emailId: email, MPIN: password }, (err, res) => {
            ...
            if (err) {
                ...
                validators.hashPassword(password, function (err, hash) {
                    if (err) {
                        ...
                        CouponCodeProductModel.updateOne({ emailId: email }, { password: hash }, (err, data) => {
                            validators.generateJWTTokenRole({ id: res[0]._id, role: res[0]?.role }, (err, token) => {
...
const updateUserById = async (id, data) => {
    return new Promise((resolve, reject) => {
        let result = CouponCodeProductModel.updateOne({ _id: id }, { $set: data }, (err, res) => {
            if (err) {
...
const updateUserByEmail = async (id, data) => {
    let result = CouponCodeProductModel.updateOne({ emailId: id }, { $set: data });
    return result
}
...
function OTP_SEND_EMAIL(req) {
    return new Promise((resolve, reject) => {
        if (req.body.emailId) {
            CouponCodeProductModel.updateOne({
                emailId: req.body.emailId
            }, { $set: { "emailIdVerified": true } }, function (err, user) {
                CouponCodeProductModel.findOne({
                    emailId: req.body.emailId
