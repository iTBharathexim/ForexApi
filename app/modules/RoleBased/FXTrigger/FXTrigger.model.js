const FXTriggerModel = require('./FXTriggerSchema').FXTriggerModel;
...
const addUser = (user) => {
    return new Promise((resolve, reject) => {
        FXTriggerModel.create(user?.data, (err, res) => {
            if (err) {
                reject(err);
            } else if (res) {
                FXTriggerModel.find({ userId: user?.data?.userId }, (err, ResFXTriggerModel) => {
                    resolve({ status: true,ResFXTriggerModel:ResFXTriggerModel, msg: "Created SucessFully...." });
                });
            } else {
                reject(null);
            }
...
const getUser = (userId) => {
    return new Promise((resolve, reject) => {
        FXTriggerModel.find({ userId: userId?.userId }, (err, res) => {
            if (err) {
                resolve(err);
            }
            resolve(res);
...
const LiveTradeAppModel = require('../LiveTradeApp/LiveTradeSchema').LiveTradeAppModel;

const updateUserById = async (id, data) => {
    return new Promise((resolve, reject) => {
        FXTriggerModel.updateOne({ userId: id }, { $set: data }, (err, res) => {
            if (err) {
                resolve({ result: res, msg: 'Somethings wrong...', status: false })
            }
            LiveTradeAppModel.find({ _id: id }, (err, res1) => {
                FXTriggerModel.find({ userId: id }, (err, FXTriggerModelRes) => {
                    resolve({ result: res1, msg: 'Sucessfully Update', status: true, data: FXTriggerModelRes })
                })
            })
        });
    });
...
const updateUserByEmail = async (id, data) => {
    let result = FXTriggerModel.updateOne({ emailId: id }, { $set: data });
    return result
}
...
function OTP_SEND_EMAIL(req) {
    return new Promise((resolve, reject) => {
        if (req.body.emailId) {
            FXTriggerModel.updateOne({
                emailId: req.body.emailId
            }, { $set: { "emailIdVerified": false } }, function (err, user) {
                FXTriggerModel.findOne({
                    emailId: req.body.emailId
