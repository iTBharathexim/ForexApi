const DeviceTrackingOnboardingScreen = require('./DeviceTrackingOnboardingScreenSchema.js').DeviceTrackingOnboardingScreenModel;
...
const addUser = (user) => {
    return new Promise((resolve, reject) => {
        DeviceTrackingOnboardingScreen.create(user, (err, res) => {
            if (err) {
                reject(err);
            } else if (res) {
...
const login = (authString) => {
    return new Promise((resolve, reject) => {
        validators.decodeAuthString(authString, (email, password) => {
            if (email && password) {
                DeviceTrackingOnboardingScreen.find({ emailId: email }, (err, res) => {
                    if (err) {
                        resolve(err);
...
const TradeAppLogin = (deviceId) => {
    return new Promise((resolve, reject) => {
        DeviceTrackingOnboardingScreen.find({ deviceId: deviceId }, (err, res) => {
            if (err) {
                resolve(err);
            }
            resolve(res);
...
const TradeAppLoginMPIN = (deviceId, data) => {
    return new Promise((resolve, reject) => {
        DeviceTrackingOnboardingScreen.updateOne({ deviceId: deviceId }, { $set: data }, (err, res) => {
            if (err) {
                resolve({ error: err });
            }
            resolve({
...
const compareDates = (d1) => {
    console.log(new Date(d1), d1, "compareDates")
    let Enddate = moment(new Date(d1)).unix();
    let dateCheck = moment(new Date()).unix();
    console.log(Enddate, dateCheck, "compareDates")
    return dateCheck <= Enddate;
};
            ...
            if (err) {
                ...
                validators.hashPassword(password, function (err, hash) {
                    if (err) {
                        ...
                        DeviceTrackingOnboardingScreen.updateOne({ emailId: email }, { password: hash }, (err, data) => {
...
const getUserDetails = () => {
    return new Promise((resolve, reject) => {
        DeviceTrackingOnboardingScreen.find({}, (err, res) => {
            if (err) {
                resolve(err);
            }
            resolve(res);
...
const updateUserById = async (id, data) => {
    return new Promise((resolve, reject) => {
        DeviceTrackingOnboardingScreen.updateOne({ _id: id }, { $set: data }, (err, res) => {
            if (err) {

                resolve({ result: res, msg: 'Somethings wrong...', status: false })
            }
            DeviceTrackingOnboardingScreen.find({ _id: id }, (err, userdata) => {
                if (err) {
                    resolve(err);
                }
...
const updateUserByEmail = async (id, data) => {
    return new Promise((resolve, reject) => {
        let result = DeviceTrackingOnboardingScreen.updateOne({ emailId: id }, { $set: data }, (err, res) => {
            if (err) {
...
function OTP_SEND_EMAIL(req) {
    return new Promise((resolve, reject) => {
        if (req.body.emailId) {
            ...
            DeviceTrackingOnboardingScreen.updateOne({
                emailId: req.body.emailId
            }, { $set: { emailIdOTP: newOTP } }, function (err, UserData) {
                DeviceTrackingOnboardingScreen.findOne({
                    emailId: req.body.emailId
