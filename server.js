...
require("dotenv").config({ path: `app/environments/.env.` + env_file[0] });
...
const RoleBaseLoginRoutes = require("./app/modules/RoleBased/RoleBaseLogin/role_base_login.routes");
const LiveTradeAppRoutes = require("./app/modules/RoleBased/LiveTradeApp/LiveTradeApp.routes");
const ContactUs = require("./app/modules/shared/contact_us/contact_us.routes");

const filterAnyTable = require("./app/modules/shared/filter&UpdateAnyTable/filterAnyTable.routes");
const UpdateAnyTable = require("./app/modules/shared/filter&UpdateAnyTable/UpdateAnyTable.routes");
const Pagination = require("./app/modules/shared/filter&UpdateAnyTable/filterAnyTablePagination.routes");
...
app1.use(async function (req, res, next) {
    
    if ((req?.url.indexOf('v1/authenticate') == -1 ||
        req?.url.indexOf('v1/user') == -1 ||
        req?.url.indexOf('v1/otp') == -1) ||
        req?.url.indexOf('v1/LiveTradeApp') == -1) {
        res.header("Access-Control-Allow-Origin", "*");
    } else {
        res.header("Access-Control-Allow-Origin", "*");
        await res.setHeader('Content-Type', 'text/html');
        await res.status(401).send(get404Error())
    }
    var s = new Date().toLocaleString(undefined, { timeZone: 'Asia/Kolkata' });
    res.setHeader('Date', s);
    next();
});

app1.use(bodyParser.json({ limit: "100mb" }));
app1.use(bodyParser.urlencoded({ limit: "100mb", extended: true, parameterLimit: 50000 }));
...
app1.use(passport.authenticate('session'));
require("./app/config/passport")(passport);

app1.use(require('./app/middlewares/authMiddleware'));
app1.use((req, res, next) => {
    next();
});
var io = require('socket.io')(httpServer,{pingInterval:10});
// io.on('connection', function(client) {
//     
//    client.emit("message", "Some thing to show");
// });

let port_assign = env_file[0] == 'local' ? 8082 : 80;
if (env_file[0] + 'dddd' == 'local') {
    httpServer.listen(port_assign, '192.168.203.20', () => {
        
    });
} else {
    httpServer.listen(port_assign, () => {
...
app1.use("/v1/RoleBaseSingInSingUp", passport.authenticate("jwt", { session: false }), RoleBaseLoginRoutes);
app1.use("/v1/LiveTradeApp", cors(), LiveTradeAppRoutes.router);
app1.use("/v1/ContactUs", passport.authenticate("jwt", { session: false }), ContactUs);

app1.use("/v1/filterAnyTable", passport.authenticate("jwt", { session: false }), filterAnyTable);
app1.use("/v1/UpdateAnyTable", passport.authenticate("jwt", { session: false }), UpdateAnyTable);
app1.use("/v1/Pagination", passport.authenticate("jwt", { session: false }), Pagination);
const couponCodeDiscount = require("./app/modules/RoleBased/CouponCodeProduct/CouponCodeProduct.routes");
app1.use("/v1/product", cors(), couponCodeDiscount);
const SubscriptionPlan = require("./app/modules/RoleBased/SubscriptionPlan/SubscriptionPlan.routes");
app1.use("/v1/SubscriptionPlan", cors(), SubscriptionPlan);

const FXMarginDetails = require("./app/modules/RoleBased/FXMarginDetails/FXMarginDetails.routes")
app1.use("/v1/FXMarginDetails", cors(), FXMarginDetails.router);

const DeviceOnboardingScreen = require("./app/modules/RoleBased/DeviceTrackingOnboardingScreen/DeviceTrackingOnboardingScreen.routes")
app1.use("/v1/DeviceOnboardingScreen", cors(), DeviceOnboardingScreen.router);

const FXTrigger = require("./app/modules/RoleBased/FXTrigger/FXTrigger.routes")
app1.use("/v1/FXTrigger", cors(), FXTrigger.router);

const RBIRef = require("./app/modules/RoleBased/RBIRef/RBIRef.routes")
app1.use("/v1/RBIRef", cors(), RBIRef.router);

const BenchMarks = require("./app/modules/RoleBased/BenchMarks/BenchMarks.routes")
app1.use("/v1/BenchMarks", cors(), BenchMarks.router);

const NetDaniaWebApi = require("./app/modules/RoleBased/NetDaniaWebApi/NetDaniaWebApi.router")
app1.use("/v1/NetDaniaWebApi", cors(), NetDaniaWebApi?.router);

const NetDaniaWebApiHistorical = require("./app/modules/RoleBased/NetDaniaWebApi/HistoricalData/NetDaniaWebApi.Historical.router")
app1.use("/v1/JsApiHistorical", cors(), NetDaniaWebApiHistorical);

dbconnection?.main().then(() => {
    const UserCtrl = require("./app/modules/RoleBased/LiveTradeApp/LiveTradeApp.model");
    const FXMarginDetailsCtrl = require("./app/modules/RoleBased/FXTrigger/FXTrigger.model");
    try {
        UserCtrl.getUserDetails().then(async (res) => {
            
            for (let index = 0; index < res?.length; index++) {
                const element = await res[index];
                if (dbconnection.CacheMemory.get(element?._id?.toString()) == undefined) {
                    await FXMarginDetailsCtrl.getUser({ userId: element?._id }).then((userres) => {
                        if (userres?.length != 0) {
                            dbconnection.CacheMemory.set(element?._id?.toString(), { triggerdata: userres, userData: element }, 10000);
                        }
                    })
                }
            }
            
            await FXTrigger.callMemoryCache(dbconnection.CacheMemory);
            await FXMarginDetails.callMemoryCache(dbconnection.CacheMemory);
            await NetDaniaWebApi.SocketLoad(io,dbconnection.CacheMemory)
            await LiveTradeAppRoutes.SocketLoad(io,dbconnection.CacheMemory)
        })
    } catch (error) {
...
app1.use('/allroute', (req, res) => {
    
    res.json({ allroute: req.body.user })
});
...
function encrypt(message) {
    ...
    return { encrypt: encryptedData, decrypt: decryptedData };
}

function get404Error() {
    return ` 
 <h1 style="margin: auto;
 text-align: center;color: red;">You are not authorized to start or access this server-side service. ${env_file[0]}</h1>
 <a>
 <div class="container">
 <div class="row">
   <div class="col-sm-12 col-md-12 mt-5 mb-5" style="margin-top: 90px;">
     <svg width="100%" height="700px" viewBox="0 0 636 324" fill="none" xmlns="http://www.w3.org/2000/svg">
<g id="OBJECTS">
<g id="Group">
